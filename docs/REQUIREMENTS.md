...
- 跟随 One API 上游（songquanpeng/one-api）持续更新，同步 bugfix 与安全补丁
- 减少对上游代码的深度定制，降低合并成本

**策略**：
- 建立上游跟踪机制（定期 merge `upstream/main`）
- 自定义改动收敛到独立文件/目录，减少与上游冲突面
...
> 变更记录：v2.4（2026-08-29）撤销 v2.3 的「智能路由完整实现」目标，改为上述 v1.2 方向。
