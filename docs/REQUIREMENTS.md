...
> **版本**: v2.4  
> **日期**: 2026-08-29  
> **状态**: v1.1.29 迭代完成；v1.2 方向已确认（保留核心转发+精简计费，不做智能路由，见第 10 章）  
...
> v1.2 方向已与用户（元成）确认（2026-08-29）：回归 One API 核心价值，精简业务逻辑。

### 10.1 保留核心 Token 转发，跟随上游更新 🔨

**方向**：核心（token 转发）跟随 One API 上游同步更新；其余业务保持深度定制。

**跟随上游（核心）**：
- 保留 One API 核心的 token 转发能力（多渠道负载均衡、健康检查、OpenAI 兼容 API）
- 定期 merge `upstream/main`，同步 bugfix 与安全补丁

**深度定制（ATR 特有）**：
- 渠道组（`channel_group`）、金额制计费、健康检查探测、帮助页等 ATR 特有业务继续自研
- 自定义改动收敛到独立文件/目录，减少与上游冲突面，便于定期合并

### 10.2 精简计费逻辑 🔨

**目标**：
- 去掉 One API 原生复杂的计费链路（`modelRatio × groupRatio × completionRatio` 倍率体系）
- 保留简单清晰的计费方式（当前金额制已基本达成）

**简化方向**：
- 统一按渠道 `model_prices`（¥/M token）计费，移除倍率链路
- 清理内部不使用的遗留业务（赠送/邀请/兑换码等）

### 10.3 不做智能 Token 路由 ✅（明确排除）

**决策**：v1.2 **不支持**智能 token 路由（内容分类路由）。

- `model="auto"` 保持当前简化行为（一律路由到默认模型）
- 不再扩展内容分类器、分组差异化路由、可配置映射等能力（**撤销 v2.3 的第 10.1 节规划**）

```mermaid
flowchart LR
    A["v1.2 方向"] --> B[跟随 One API 上游]
    A --> C[精简计费逻辑]
    A --> D[不做智能路由]
    B --> E[保留核心 Token 转发]
    C --> F[统一按 model_prices 计费]
    D --> G["auto 一律路由默认模型"]
```

> 变更记录：v2.4（2026-08-29）撤销 v2.3 的「智能路由完整实现」目标，改为上述 v1.2 方向；新增 10.4 用户 Token 摘要审计。

### 10.4 用户 Token 摘要审计 🔨

**需求**：记录每个用户的 token 使用摘要，为后续用户使用审计提供依据。

**目标**：
- 按用户维度记录 token 用量（请求次数、输入/输出 token 数）
- 支持按时间（日/月）与模型维度聚合
- 管理后台提供「用户用量审计」查看

**实现方向**：
- 复用现有 `logs` 表的请求级用量数据（`prompt_tokens` / `completion_tokens`）
- 用量摘要聚合：按 **用户 × 日期 × 模型** 维度（定时聚合任务或查询时聚合）
- 管理后台新增「用户用量审计」页面（表格 + 汇总）
