...
| 模块 | 功能 |
|------|------|
| 🔀 **智能路由** | `model` 传 `"auto"` 或不传时自动路由到默认模型（v1.2 明确不做内容分类路由） |
...
> **简化实现**：`model="auto"` 或不传时一律路由到默认模型；v1.2 明确不做内容分类路由（详见 [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) 第 10 章）。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/v1/chat/completions` | 聊天补全（支持 SSE 流式），`model:"auto"` 路由到默认模型 |
