...
| 模块 | 功能 |
|------|------|
| 🔀 **智能路由** | `model` 传 `"auto"` 或不传时自动路由到默认模型；内容分类路由到不同模型列为下一大版本目标 |
...
用户请求时 `model` 参数传 `"auto"` 或不传，系统自动路由到默认模型（当前为 `qwen-max`）。

> **当前为简化实现**：内容分类器已就绪（可识别代码/推理/办公场景），但「代码推理」与「日常办公」暂映射到同一默认模型，分类结果暂不影响最终模型。「按内容分类路由到不同模型」（如代码→高级模型、办公→基础模型）已列为下一大版本目标，详见 [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md)。
